...
function mapRowToEntity(row: any): UserEntity {
  ...
  const parsedCurrentTask =
    typeof currentTaskValue === "number"
      ? currentTaskValue
  ...
  return UserEntity.create({
    ...
    currentTask:
      currentTaskValueOrUndefined !== undefined
        ? CurrentTaskId.create(currentTaskValueOrUndefined)
        : undefined,
    currentTaskTime:
      currentTaskTimeValue !== undefined
...
export class PostgresUserRepository implements UserRepository {
  ...
  async create(user: UserEntity): Promise<UserEntity> {
    ...
    const result = await query(
      "INSERT INTO users (id, display_name, icon_path, current_task, current_task_time) VALUES ($1, $2, $3, $4, $5) RETURNING *",
      [
        persistence.id,
