...
export class PostgresTaskRepository implements TaskRepository {
    async findAllByUser(userId: TaskOwnerId, includeComplete: boolean): Promise<TaskEntity[]> {
        ...
        const result = await query('SELECT * FROM tasks WHERE user_id = $1 ' + (includeComplete ? '' : 'AND is_complete = false ') + 'ORDER BY id ASC', [userId.value]);
        return result.rows.map(mapRowToEntity);
    }

    async findByIdForUser(id: TaskId, userId: TaskOwnerId): Promise<TaskEntity | null> {
        if (process.env.NODE_ENV === 'production') {
            const result = await sql`SELECT * FROM tasks WHERE id = ${id.value} AND user_id = ${userId.value}`;
            return result.rows[0] ? mapRowToEntity(result.rows[0]) : null;
        }
        const result = await query('SELECT * FROM tasks WHERE id = $1 AND user_id = $2', [id.value, userId.value]);
        return result.rows[0] ? mapRowToEntity(result.rows[0]) : null;
    }
    ...
    async create(task: TaskEntity): Promise<TaskEntity> {
        ...
        if (process.env.NODE_ENV === 'production') {
            const result = await sql`INSERT INTO tasks (user_id, task_name, deadline, total_set, current_set, is_complete) VALUES(${persistence.userId}, ${persistence.name}, ${persistence.deadline}, ${persistence.totalSet}, ${persistence.currentSet}, ${persistence.isComplete}) RETURNING *`;
            return mapRowToEntity(ensureRow(result, 'Task not found'));
        }
        const result = await query('INSERT INTO tasks (user_id, task_name, deadline, total_set, current_set, is_complete) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [persistence.userId, persistence.name, persistence.deadline, persistence.totalSet, persistence.currentSet, persistence.isComplete]);
        return mapRowToEntity(ensureRow(result, 'Task not found'));
    }
    ...
    async update(task: TaskEntity): Promise<TaskEntity> {
        ...
        if (process.env.NODE_ENV === 'production') {
            const result = await sql`UPDATE tasks SET task_name = ${persistence.name}, deadline = ${persistence.deadline}, total_set = ${persistence.totalSet}, current_set = ${persistence.currentSet}, is_complete = ${persistence.isComplete} WHERE id = ${persistence.id} AND user_id = ${persistence.userId} RETURNING *`;
            return mapRowToEntity(ensureRow(result, 'Task not found'));
        }
        const result = await query('UPDATE tasks SET task_name = $1, deadline = $2, total_set = $3, current_set = $4, is_complete = $5 WHERE id = $6 AND user_id = $7 RETURNING *', [persistence.name, persistence.deadline, persistence.totalSet, persistence.currentSet, persistence.isComplete, persistence.id, persistence.userId]);
        return mapRowToEntity(ensureRow(result, 'Task not found'));
    }

    async delete(id: TaskId, userId: TaskOwnerId): Promise<void> {
        if (process.env.NODE_ENV === 'production') {
            const result = await sql`DELETE FROM tasks WHERE id = ${id.value} AND user_id = ${userId.value}`;
            if ((result.rowCount ?? 0) === 0) {
                throw new NotFoundError('Task not found');
            }
            return;
        }
        const result = await query('DELETE FROM tasks WHERE id = $1 AND user_id = $2', [id.value, userId.value]);
        if ((result.rowCount ?? 0) === 0) {
            throw new NotFoundError('Task not found');
        }
    }
