...
export class PostgresTaskRepository implements TaskRepository {
    ...
    async create(task: TaskEntity): Promise<TaskEntity> {
        if (process.env.NODE_ENV === 'production') {
            const result = await sql`INSERT INTO tasks (user_id, task_name, deadline, total_set, current_set, is_complete) VALUES(${task.userId}, ${task.name}, ${task.deadline}, ${task.totalSet}, ${task.currentSet}, ${task.isComplete}) RETURNING *`;
            return mapRowToEntity(ensureRow(result, 'TaskNotFound'));
        }
        const result = await query('INSERT INTO tasks (user_id, task_name, deadline, total_set, current_set, is_complete) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [task.userId, task.name, task.deadline, task.totalSet, task.currentSet, task.isComplete]);
        return mapRowToEntity(ensureRow(result, 'TaskNotFound'));
    }

    async update(task: TaskEntity): Promise<TaskEntity> {
        if (process.env.NODE_ENV === 'production') {
            const result = await sql`UPDATE tasks SET task_name = ${task.name}, deadline = ${task.deadline}, total_set = ${task.totalSet}, current_set = ${task.currentSet}, is_complete = ${task.isComplete} WHERE id = ${task.id} AND user_id = ${task.userId} RETURNING *`;
            return mapRowToEntity(ensureRow(result, 'TaskNotFound'));
        }
        const result = await query('UPDATE tasks SET task_name = $1, deadline = $2, total_set = $3, current_set = $4, is_complete = $5 WHERE id = $6 AND user_id = $7 RETURNING *', [task.name, task.deadline, task.totalSet, task.currentSet, task.isComplete, task.id, task.userId]);
        return mapRowToEntity(ensureRow(result, 'TaskNotFound'));
    }
