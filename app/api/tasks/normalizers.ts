...
export function normalizeOptionalDeadline(fieldName: string, value: unknown): string | undefined {
...
export function normalizePositiveInteger(fieldName: string, value: unknown): number {
    const numeric = typeof value === 'number' ? value : typeof value === 'string' ? Number(value) : NaN;
    if (!Number.isInteger(numeric) || numeric < 1) {
        throw new BadRequestError(`${fieldName} must be an integer greater than or equal to 1`);
    }
    return numeric;
}

export function normalizeNonNegativeInteger(fieldName: string, value: unknown): number {
    const numeric = typeof value === 'number' ? value : typeof value === 'string' ? Number(value) : NaN;
    if (!Number.isInteger(numeric) || numeric < 0) {
        throw new BadRequestError(`${fieldName} must be an integer greater than or equal to 0`);
    }
    return numeric;
}
