...
function ensureUserIdMatch(sessionId: string, requestedId: string) {
  if (sessionId !== requestedId) {
    throw new Response(JSON.stringify({ error: 'Forbidden: user id does not match session user id' }), { status: 403 });
  }
}
...
export async function GET(request: Request, { params }: { params: { userId: string } }) {
  ...
  if (!requestedId) {
    return Response.json({ error: 'Bad Request: invalid user id' }, { status: 400 });
  }
...
export async function PUT(request: Request, { params }: { params: { userId: string } }) {
  ...
  const { display_name, icon_path, current_task, current_task_time } = body ?? {};
